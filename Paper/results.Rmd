...
tmp <- ave_df[ave_df$Imputation == "MCAR", ]
...
dd2$Imputation <- factor(dd2$Imputation, levels = c("MCAR", "MAR", "MARwR"))         
ave2 <- ave_df[, c(1, 2, 7)]
ave2 <- subset(ave2, Method != "gb")

ggplot(dd2, aes(x = Method, y = value)) + 
  geom_point(color = "lightblue", size = 2) + 
  geom_point(data = melt(ave2, id.vars = c("Method", "Imputation")),
            color = "red", size = 3) + 
...
dev.copy2eps(file = "iAUC.eps", width = 10, height = 4)
```
