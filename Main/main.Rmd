...
discard <-  c(
  ## Variables that are mostly or entirely missing in the training data
  "HGTBLCAT", 
  "WGTBLCAT", 
  "HEAD_AND_NECK", 
  "PANCREAS", 
...
trainModels <- function(train, 
  ...
  results$varSelectMCAR <- as.character(with(stabSelectMCAR, var[freq > selectThres]))
  results$varSelectMAR <- as.character(with(stabSelectMAR, var[freq > selectThres]))
  results$varSelectMARwR <- as.character(with(stabSelectMARwR, var[freq > selectThres]))
  ...
  if(trace) cat("Lasso\n")
    ...
    results$lassoMARwR <- lasso(trainMARwR, test = testMARwR)
  ...
  if("GBfull" %in% methods) {
    ...
    results$gbFullMCAR <- gradBM(trainMCAR, testMCAR, ntrees = 1000, shrinkage = 1, verbose = trace, full = TRUE)
    results$gbFullMAR <- gradBM(trainMAR, testMAR, ntrees = 1000, shrinkage = 1, verbose = trace, full = TRUE)
  }
